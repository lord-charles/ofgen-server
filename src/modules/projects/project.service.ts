...
export class ProjectService {
  ...
  async create(createDto: CreateProjectDto): Promise<Project> {
    ...
    if (createDto.subcontractors && createDto.subcontractors.length > 0) {
      projectData.subcontractors = createDto.subcontractors.map(
  ...
  async update(id: string, updateDto: UpdateProjectDto): Promise<Project> {
    ...
    if (updateDto.location)
      updateData.location = new Types.ObjectId(updateDto.location);
    // if (updateDto.serviceOrder)
    //   updateData.serviceOrder = new Types.ObjectId(updateDto.serviceOrder);
    if (updateDto.projectLeader)
      updateData.projectLeader = new Types.ObjectId(updateDto.projectLeader);

    // Handle arrays of ObjectIds
    if (updateDto.subcontractors && updateDto.subcontractors.length > 0) {
      updateData.subcontractors = updateDto.subcontractors.map(
        (id) => new Types.ObjectId(id),
      );
    }
  ...
  async addSubcontractor(
    ...
    try {
      ...
      if (
        ...
        project.subcontractors.some((id) => id.equals(subId))
      ) {
   ...
   */
  async getProjectsBySubcontractor(
    subcontractorId: string,
  ): Promise<Project[]> {
    try {
      const subId = new Types.ObjectId(subcontractorId);
    ...
    if (data.milestones && data.milestones.length > 0) {
      data.milestones = data.milestones.map((milestone) => {
        ...
        if (milestone.completedDate)
          processedMilestone.completedDate = new Date(milestone.completedDate);
        ...
        if (milestone.tasks && milestone.tasks.length > 0) {
          processedMilestone.tasks = milestone.tasks.map((task) => {
            ...
            if (task.assignedSubcontractor) {
              processedTask.assignedSubcontractor = new Types.ObjectId(
                task.assignedSubcontractor,
              );
            }
    ...
    if (data.risks && data.risks.length > 0) {
      data.risks = data.risks.map((risk) => {
        ...
        if (risk.identifiedDate)
          processedRisk.identifiedDate = new Date(risk.identifiedDate);
        if (risk.targetResolutionDate)
          processedRisk.targetResolutionDate = new Date(
