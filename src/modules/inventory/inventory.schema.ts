...
import { IsEnum, IsOptional, IsString, IsNumber, IsDate, IsArray, IsBoolean, Min, Max } from 'class-validator';
...
export class StockLevel {
    ...
    @ApiProperty({ description: 'Reserved quantity (allocated but not yet consumed)' })
...
export class InventoryItem {
    ...
    @ApiProperty({ description: 'Alternative item codes (manufacturer part numbers, etc.)' })
    ...
    @ApiProperty({ description: 'Stock levels across locations', type: [StockLevel] })
    ...
    @ApiProperty({ description: 'Whether item is serialized/tracked individually' })
    @Prop({ default: false })
    @IsBoolean()
    isSerialized: boolean;

    @ApiProperty({ description: 'Whether item is a service rather than physical item' })
    @Prop({ default: false })
    @IsBoolean()
    isService: boolean;
    ...
    @ApiProperty({ description: 'Document attachments (datasheets, manuals, etc.)' })
    @Prop({ type: [String], default: [] })
    @IsArray()
    attachments: string[];
...
InventoryItemSchema.virtual('totalStock').get(function () {
    return this.stockLevels?.reduce((total, level) => total + level.currentStock, 0) || 0;
});

InventoryItemSchema.virtual('totalReserved').get(function () {
    return this.stockLevels?.reduce((total, level) => total + level.reservedStock, 0) || 0;
});

InventoryItemSchema.virtual('totalAvailable').get(function () {
    return this.stockLevels?.reduce((total, level) => total + level.availableStock, 0) || 0;
});

// Middleware to update stock status based on levels
InventoryItemSchema.pre('save', function () {
    const totalStock = this.stockLevels?.reduce((total, level) => total + level.currentStock, 0) || 0;
    const minLevel = Math.min(...(this.stockLevels?.map(level => level.minimumLevel) || [0]));
...
InventoryItemSchema.pre('save', function () {
    this.stockLevels?.forEach(level => {
        level.availableStock = Math.max(0, level.currentStock - level.reservedStock);
    });
});
