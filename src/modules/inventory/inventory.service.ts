    ...
    try {
      const [
              ...
              isLowStock: {
                $anyElementTrue: {
                  $map: {
                    ...
                    as: 'level',
                    in: {
                      $lte: ['$$level.currentStock', '$$level.minimumLevel'],
                    },
                  },
                },
              },
        ...
        this.inventoryModel.aggregate([
          { $match: { isActive: true } },
          { $unwind: '$stockLevels' },
          { $match: { 'stockLevels.currentStock': 0 } },
          { $group: { _id: '$_id' } },
          { $count: 'count' },
        ]),
        ...
        // Top selling items (based on outgoing transactions)
        this.transactionModel.aggregate([
          {
            $match: {
              transactionType: { $in: ['SALE', 'TRANSFER_OUT'] },
              transactionDate: {
      ...
      const outOfStockCount =
        outOfStockResult.length > 0 ? outOfStockResult[0].count : 0;
      ...
      const dashboardStats: DashboardStatsDto = {
        ...
        monthlyTransactions: monthlyTransactionsResult.length,
